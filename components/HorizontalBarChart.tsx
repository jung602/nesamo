...
const HorizontalBarChart: React.FC<HorizontalBarChartProps> = ({ data, maxValue }) => {
  return (
      ...
      {data.map((item, index) => (
        <div key={item.tag} className="flex items-center m-2">
          <div className="relative flex-grow h-8 rounded overflow-hidden">
            <div className="absolute top-1/2 -translate-y-1/2 ml-2 text-white z-10">{item.tag}</div>
            <motion.div
